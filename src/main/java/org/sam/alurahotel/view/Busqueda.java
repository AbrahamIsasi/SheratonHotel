...
public class Busqueda extends JFrame {
	...
	public Busqueda() {
		...
		lblNewLabel_2.setIcon(new ImageIcon(Busqueda.class.getResource("/imagenes/aH-150px.png")));
		lblNewLabel_2.setBounds(50, 40, 190, 110);//56-51
		...
		JPanel btnExportar = new JPanel();
		btnExportar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				org.sam.alurahotel.view.ExportarDatos exportarDatos = new org.sam.alurahotel.view.ExportarDatos();

				try {
					List<Reserva> reservas = reservaController.listarTodasReservas();
					exportarDatos.exportarExcelReservas(reservas);
					JOptionPane.showMessageDialog(null, "Datos de reservas exportados exitosamente a Reservas.xlsx");
				} catch (IOException ex) {
					JOptionPane.showMessageDialog(null, "Error al exportar datos: " + ex.getMessage());
					ex.printStackTrace();
				}
			}
		});

		JPanel btnExportar1 = new JPanel();
		btnExportar1.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				org.sam.alurahotel.view.ExportarDatos exportarDatos = new org.sam.alurahotel.view.ExportarDatos();

				try {
					List<Huesped> huespedes = huespedController.listarTodosHuespedes();
					exportarDatos.exportarExcelHuespedes(huespedes);
					JOptionPane.showMessageDialog(null, "Datos de huéspedes exportados exitosamente a Huespedes.xlsx");
				} catch (IOException ex) {
					JOptionPane.showMessageDialog(null, "Error al exportar datos: " + ex.getMessage());
					ex.printStackTrace();
				}
		...
		btnExportar.setLayout(null);
		btnExportar.setBackground(new Color(12, 138, 199));
		btnExportar.setBounds(20, 508, 122, 35);
		btnExportar.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
		contentPane.add(btnExportar);

		JLabel lblExportar = new JLabel("Exp. Reservas");
		lblExportar.setHorizontalAlignment(SwingConstants.CENTER);
		lblExportar.setForeground(Color.WHITE);
		lblExportar.setFont(new Font("Ubuntu", Font.PLAIN, 18));
		lblExportar.setBounds(0, 0, 122, 35);
		btnExportar.add(lblExportar);
		setResizable(false);

		btnExportar1.setLayout(null);
		btnExportar1.setBackground(new Color(12, 138, 199));
		btnExportar1.setBounds(152, 508, 122, 35);
		btnExportar1.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
		contentPane.add(btnExportar1);

		JLabel lblExportar1 = new JLabel("Exp. Huespedes");
		lblExportar1.setHorizontalAlignment(SwingConstants.CENTER);
		lblExportar1.setForeground(Color.WHITE);
		lblExportar1.setFont(new Font("Ubuntu", Font.PLAIN, 18));
		lblExportar1.setBounds(0, 0, 122, 35);
		btnExportar1.add(lblExportar1);
		setResizable(false);
